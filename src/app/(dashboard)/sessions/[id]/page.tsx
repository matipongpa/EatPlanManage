...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const mealSession = await db.mealSession.findUnique({ where: { id: params.id } })
...
export default async function SessionDetailPage({ params }: PageProps) {
  ...
  const totalVotes = mealSession.options.reduce((sum, o) => sum + o._count.votes, 0)
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge className={cn('border', getStatusColor(mealSession.status))} variant="outline">
              {getStatusLabel(mealSession.status)}
            </Badge>
          </div>
          {mealSession.description && (
            <p className="mt-1 text-sm text-muted-foreground">{mealSession.description}</p>
          )}
            ...
            <CardContent>
              {isMember ? (
                ...
                <div className="rounded-xl border border-dashed p-8 text-center text-muted-foreground">
                  <p className="text-sm">Join this session to see and vote on options.</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <Card className="border-green-200 bg-green-50/40">
              ...
              <CardContent className="space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-semibold">{mealSession.appointment.restaurantName}</span>
                </div>
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  {mealSession.appointment ? 'Update Appointment' : 'Set Appointment'}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <AppointmentForm
                ...
                <div key={member.id} className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    {member.userId === mealSession.ownerId && (
                      <p className="text-[11px] text-orange-600 mt-0.5">Owner</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              <CardContent className="pt-4">
                <div className="flex gap-2.5">
                  ...
                  <p className="text-xs text-orange-700">
                    Vote for your preferred restaurant. You can change your vote anytime before voting closes.
                  </p>
                </div>
              </CardContent>
            </Card>
